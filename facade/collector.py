...
def _load_embedder() -> Optional[SentenceTransformer]:
    ...
    May return ``None`` when :mod:`sentence_transformers` is unavailable.
    """
    global _EMBEDDER
...
class Metrics:
    ...
    def __init__(self, cfg: Dict[str, Any], embedder: Optional[SentenceTransformer]) -> None:
        self.cfg = cfg
    ...
    def grv(self, answer: str, *, mode: str = "simple") -> float:
        if mode != "simple":
            raise NotImplementedError("only 'simple' supported")
        ...
        vocab = set(tokens)
        score = 0.0
        cap = self.cfg.get("MAX_VOCAB_CAP", MAX_VOCAB_CAP)
        if vocab:
            score = log1p(len(vocab)) / log1p(cap)
...
def api_key_check(service: str, key: Optional[str]) -> Optional[str]:
    ...
    # TODO: validate prefix and length for each service
    return None

def _dummy_response(question: str, **_: Any) -> str:
    """Return a deterministic fallback response.

    Extra keyword arguments are ignored so the signature matches other providers.
    """
...
def get_ai_response(question: str, provider: str | None = None) -> str:
    ...
    # Optional → AIHandler に絞ってから呼び出すと mypy が型を推論できる
    handler: AIHandler | None = AI_PROVIDERS.get(prov)
    if handler is not None:
        return handler(question, role="answer")
...
def run_cycle(
    ...
    *,
    ...
    q_prov = q_provider
    metrics = Metrics(cfg, _load_embedder())
...
def main(argv: List[str] | None = None) -> None:
    ...
    if args.exp_id:
        ...
        # TODO: truncate or hash when > 200 chars
